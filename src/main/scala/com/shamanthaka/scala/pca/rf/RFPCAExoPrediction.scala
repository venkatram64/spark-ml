...
object RFPCAExoPrediction extends App{

  val sparkSession = SparkSession
    .builder()
    .master("local")
    .appName("RFPCAExoPrediction")
    .getOrCreate()

  val testData = sparkSession.read.format("libsvm").load("exo_libsvm_test_data.txt")
  //show schema
  ...
  val model = PipelineModel.load("rfPCAExoModel")
